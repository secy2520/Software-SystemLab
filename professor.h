...
void view_offered(int clientSocket)
{
    ...
    while (read(dbFile, &course_record, sizeof(struct course)) > 0)
    {
        if (strcmp(courseid, course_record.course_id) == 0)
        {
            ...
            course_found = true;
            char courseDetails[1000];
            snprintf(courseDetails, sizeof(courseDetails), "Course ID: %s\nCourse Name: %s\nDepartment: %s\nMax Strength: %d\nCurrent Strength: %d\n",
            course_record.course_id, course_record.name, course_record.department, course_record.max_strength, course_record.curr_strength);
...
bool update_course(int clientSocket)
{
    ...
    if (bytesRead == -1 || bytesRead < sizeof(course_id))
    {
    ...
    // Iterate through the file to find the course record
    struct course course_record;
    bool found = false;
